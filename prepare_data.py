...
from training.utilities import *
...
if __name__ == "__main__":
    ...
    # Add edge labels to your labeled data
        ...
        if not os.path.isdir(labels_folder):
            raise Exception(labels_folder + " is not a directory. Inputs must be a folder of files. Please refer to readme for more info")
        ...
        for label_name in os.listdir(labels_folder):
            ...
            edge_vol = add_edge_labels(vol)
            write_tiff_stack(edge_vol, os.path.join(output_folder, label_name))

    else:
        raise Exception("You must choose your type of preparation (generate_validation_set, generate_training_set, add_edge_labels)")
