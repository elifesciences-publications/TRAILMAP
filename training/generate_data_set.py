...
def crop_box(x, y, z, vol, length):
    return vol[z:z + length, x:x + length, y:y + length]
...
def get_random_training(volume, label):
    ...
    volume_chunk = crop_box(x, y, z, volume, input_dim)
    label_chunk = crop_box(x, y, z, label, input_dim)
...
def draw_progress_bar(percent, bar_len = 40):
    ...
    sys.stdout.write("\r")
    sys.stdout.write("[{:<{}}] {:>3.0f}%".format("=" * int(bar_len * percent), bar_len, percent * 100))
    sys.stdout.flush()
